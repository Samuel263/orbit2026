...
import type { Lang } from "@/lib/content.functions";
...
export function WhatsAppButton({ settings, lang = "es" }: Props) {
  const [hovering, setHovering] = useState(false);
  const number = String((settings?.whatsapp_number as string) ?? "").replace(/[^0-9]/g, "") || "56994101429";
  const messages = (settings?.whatsapp_message as Record<string, string> | undefined) ?? {};
  ...
  return (
    ...
    >
      <svg className="wa-btn__ring" viewBox="0 0 200 200" aria-hidden="true">
        <defs>
          <path id="wa-circle" d="M100,100 m-74,0 a74,74 0 1,1 148,0 a74,74 0 1,1 -148,0" />
        </defs>
        <text fill="#ffffff" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif", fontSize: "16px", fontWeight: 700, letterSpacing: "3px" }}>
          <textPath href="#wa-circle" startOffset="0">CONTACTAR • CONTACTAR • CONTACTAR • CONTACTAR •</textPath>
        </text>
      </svg>
      <span className={`wa-btn__circle ${hovering ? "wa-btn__circle--pop" : "wa-btn__circle--unpop"}`} aria-hidden="true" />
      <span className={`wa-btn__icon ${hovering ? "wa-btn__icon--pop" : "wa-btn__icon--unpop"}`} aria-hidden="true">
